        ...
        <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12 pb-32 relative">
                        ...
                        >
                            {showFeeModal.type === 'withdrawalFee' && (
                                <>
                                    ...
                                    <Link href="/dashboard/deposit" className="block w-full bg-white hover:bg-blue-500 text-black hover:text-white font-bold uppercase tracking-widest py-4 rounded-xl transition-all">
                                        Deposit Fee Now
                                    </Link>
                                </>
                            )}
            ...
            <div className={`mb-10 text-center relative transition-opacity ${showFeeModal ? 'opacity-0' : 'opacity-100'}`}>
                ...
                <div className="mt-4 flex items-center justify-center gap-2">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? "w-8 bg-blue-500" : i < step ? "w-4 bg-blue-500/40" : "w-4 bg-white/10"}`} />
                    ))}
                </div>
            </div>
                        ...
                        >
                            ...
                            <div className="relative mb-8">
                                ...
                                <input
                                    ...
                                    min="1"
                                    max={userBalance}
                                    className="w-full bg-black/40 border border-white/10 rounded-xl py-6 pl-12 pr-6 text-3xl font-bold text-white outline-none focus:border-blue-500/50 transition-colors"
                                />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full bg-white text-black font-bold uppercase tracking-widest py-4 rounded-xl hover:bg-blue-500 hover:text-white transition-all disabled:opacity-50 disabled:hover:bg-white disabled:hover:text-black"
                            >
                                Continue
                            </button>
                        </motion.div>
                    ...
                    {step === 2 && (
                        ...
                        >
                                ...
                                {method && method.id !== 'bank' && (
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <input type="text" value={bankName} onChange={(e) => { setBankName(e.target.value); setErrorMsg(""); }} placeholder="e.g. Chase Bank, Bank of America" className="w-full bg-black/60 border border-white/10 rounded-lg py-4 px-4 text-sm tracking-wide text-white outline-none focus:border-blue-500/50 transition-colors" />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] uppercase tracking-widest text-white/50 font-bold mb-2">Account Name</label>
                                            <input type="text" value={accountName} onChange={(e) => { setAccountName(e.target.value); setErrorMsg(""); }} placeholder="e.g. John Doe" className="w-full bg-black/60 border border-white/10 rounded-lg py-4 px-4 text-sm tracking-wide text-white outline-none focus:border-blue-500/50 transition-colors" />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] uppercase tracking-widest text-white/50 font-bold mb-2">Account Number / IBAN</label>
                                            <input type="text" value={accountNumber} onChange={(e) => { setAccountNumber(e.target.value); setErrorMsg(""); }} placeholder="Enter account number or IBAN..." className="w-full bg-black/60 border border-white/10 rounded-lg py-4 px-4 text-sm tracking-wide text-white outline-none focus:border-blue-500/50 transition-colors" />
                                        </div>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            <button
                                ...
                                className="w-full bg-white text-black font-bold uppercase tracking-widest py-4 rounded-xl hover:bg-blue-500 hover:text-white transition-all disabled:opacity-50 disabled:hover:bg-white disabled:hover:text-black mt-2"
                            >
                                Secure Withdrawal
                            </button>
                        </motion.div>
                    ...
                    {step === 3 && (
                        ...
                        >
                            <h2 className="text-lg font-bold text-white mb-2 uppercase tracking-widest flex items-center gap-2">
                                <ShieldCheck className="w-5 h-5 text-blue-500" /> Security PIN
                            </h2>
                            ...
                            <div className="flex gap-4 mb-8">
                                {[0, 1, 2, 3].map((i) => (
                                    <div
                                        key={i}
                                        className={`w-4 h-4 rounded-full transition-all duration-300 ${pin.length > i ? 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)] scale-110' : 'bg-white/10'}`}
                                    />
                                ))}
                            </div>
                            ...
                            <button
                                ...
                                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold uppercase tracking-widest py-4 rounded-xl transition-all disabled:opacity-50 flex justify-center items-center gap-2"
                            >

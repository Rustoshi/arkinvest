        ...
        try {
            if (transaction.type === "deposit") {
                if (action === "approved") {
                    await sendEmail({
                        to: user.email,
                        subject: "ARK Invest — Deposit Approved",
                        htmlbody: buildDepositApprovedEmail(user.firstName, amountStr, method),
                    });
                } else {
                    await sendEmail({
                        to: user.email,
                        subject: "ARK Invest — Deposit Not Approved",
                    ...
                    if (referral) {
                        ...
                        if (referrer) {
                            ...
                            try {
                                await sendEmail({
                                    to: referrer.email,
                                    subject: "ARK Invest — Referral Bonus Credited!",

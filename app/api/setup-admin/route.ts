...
export async function GET() {
    try {
        ...
        const existingAdmin = await User.findOne({ email: "admin@arkinvests.pro" });

        if (existingAdmin) {
            return NextResponse.json({ message: "Admin already exists", email: "admin@arkinvests.pro", password: "adminpassword" });
        }

        // Create the Super Admin
        const newAdmin = await User.create({
            email: "admin@arkinvests.pro",
            firstName: "Admin",
            lastName: "User",
            gender: "Male",
